...
{
    public partial class Urc_DinhMucImportExcel
    {

        string[] arrcolumncheck = new string[] { "MaHang", "SLQuyDoi", "KhuVuc", "DinhMucHaoHut","GhiChu","ChatLuong" };
        ...
        private void ConvertDataTableToClassList(DataTable dt)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                nvlHangHoaShow.KhuVuc = (row["KhuVuc"] != DBNull.Value) ? row["KhuVuc"].ToString() : "";
                ...
                {
                    if (double.TryParse(row["DinhMucHaoHut"].ToString(), out d))
                    {
                        nvlHangHoaShow.DinhMucHaoHut = d;
                    }
                    else
                        nvlHangHoaShow.DinhMucHaoHut = 1;
                }
        ...
        {
            string[] arrcheckempty = new string[] { "MaHang", "KhuVuc" };
            string[] arrchecnumber = new string[] { "SLQuyDoi", "DinhMucHaoHut" };
            double d = 0;
            int i = 1;
            ...
            if (lstdata.Count == 0)
                return false;
            //Kiểm tra trùng lặp
            var querycheckduplicate = lstdata.GroupBy(p => new { MaHang = p.MaHang, KhuVuc = p.KhuVuc })
                .Select(p => new { MaHang = p.Key.MaHang, KhuVuc = p.Key.KhuVuc, count = p.Count() }).Where(p => p.count > 1);
            if(querycheckduplicate.Any())
            {
                ...
                foreach (var item in lstdata)
                {
                    ...
                    {
                        if (it.KhuVuc == item.KhuVuc && it.MaHang == item.MaHang)
                        {
                            item.Err = "Mã hàng và khu vực này bị trùng lặp.";
                            break;
                        }
        ...
        private async Task saveAsync()
        {
            ...
            foreach (DataDropDownList art in Artselected)
            {
                foreach(var it in lstdata)
                {
                    ...
                    rownew["MaSP"] = sanPhamShowcrr.MaSP;
