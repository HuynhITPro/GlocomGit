...
{
    public partial class Urc_KhachHangAddItem
    {
        [Inject]
        ToastService toastService { get; set; }
        [Inject] PhanQuyenAccess phanQuyenAccess { get; set; }

        private bool checklogic(KhachHangNVLShow khachHangNVLShow)
        {
        ...
        protected override Task OnInitializedAsync()
        {
            ...
            visibleedit = !string.IsNullOrEmpty(khachHangNVLShow.MaKH);
        ...
        private async void CustomValidation(object? sender, ValidationRequestedEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(khachHangNVLShow.MaKH))
            {
                validationMessages.Add(() => khachHangNVLShow.MaKH, "Mã khách hàng không được để trống");
                
               
            }
            if (string.IsNullOrEmpty(khachHangNVLShow.TenKH))
            {
                validationMessages.Add(() => khachHangNVLShow.TenKH, "Tên khách hàng không được để trống");
               
            }
            if (string.IsNullOrEmpty(khachHangNVLShow.NhomKH))
            {
                validationMessages.Add(() => khachHangNVLShow.NhomKH, "Nhóm khách hàng không được để trống.");
            }
        ...
        private async Task saveAsync()
        {
            ...
            string sql = "NVLDB.dbo.NvlKhachHang_Insert";
            List<ParameterDefine> lstpara = new List<ParameterDefine>();
            lstpara.Add(new ParameterDefine("@NhomKH", khachHangNVLShow.NhomKH));
            lstpara.Add(new ParameterDefine("@MaKH", khachHangNVLShow.MaKH));
            lstpara.Add(new ParameterDefine("@TenKH", khachHangNVLShow.TenKH));
        ...
        private async Task updateAsync()
        {
            ...
            CallAPI callAPI = new CallAPI();
            string sql = "NVLDB.dbo.NvlKhachHang_Update";
            List<ParameterDefine> lstpara = new List<ParameterDefine>();
            lstpara.Add(new ParameterDefine("@NhomKH", khachHangNVLShow.NhomKH));
