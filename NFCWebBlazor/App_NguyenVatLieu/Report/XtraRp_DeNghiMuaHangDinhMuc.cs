...
{
    public partial class XtraRp_DeNghiMuaHangDinhMuc : DevExpress.XtraReports.UI.XtraReport
    {
        public XtraRp_DeNghiMuaHangDinhMuc()
        {
            InitializeComponent();
        ...
        public void setNguoiDuyet(string phongban, string LoaiKeHoach, string nguoilap, string nguoikiem, string nguoiduyet, string DaDuyet, string NoiDungDeNghi,DateTime? Ngay)
        {
            if (string.IsNullOrEmpty(NoiDungDeNghi))
            {
                ...
                //if (LoaiKeHoach.Contains("Xuat"))
                //    this.Parameters["NoiDungDeNghi"].Value = string.Format("{0} đề nghị P. Cung Tiêu xuất kho vật tư phục vụ cho sản xuất như sau:", phongban);
        ...
        {
            //XRTableCell richText = sender as XRTableCell;
            //if (richText.Text.Contains("ngoài kế hoạch"))
            //{
            //    xrSoKH.Text = string.Format("Số KH: Đề nghị ngoài kế hoạch");
            //}
            //else
            //    xrSoKH.Text = richText.Text;
        }
    }
}
