...
{
    public partial class XtraRp_DuyetGiaDetailItem : DevExpress.XtraReports.UI.XtraReport
    {
        public XtraRp_DuyetGiaDetailItem()
        {
            InitializeComponent();
            //setInitList();
        }
        ...
        public void setInitList(List<NvlDuyetGiaItemShow> lst)
        {
            ...
            dt.Columns.Add("DonGiaDuyet", typeof(string));
            ...
            foreach(var it in querylst)
            {
                if(SerialOld!=it.Serial)
                {
                    ...
                    rownew["DonGiaDuyet"] = ((it.DonGiaDuyet > 0) ? ("Giá duyệt: " + it.DonGiaDuyet.Value.ToString("#,#.###")):"");
            ...
            //    {
            //        Console.WriteLine(string.Format("{0}:{1}",dt.Columns[j].ColumnName, dt.Rows[k][j]));
            //    }
            //}
            ...
            int i = 0;
            if (query.Count == 4)//Nếu =4 thì cho cột nhỏ lại 1 chút để chứa đủ 4 nhà
                WidthCell = 200F;
            foreach (var it in query)
            {
