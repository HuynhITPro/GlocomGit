...
{
    public partial class View_DonHang_ChuaNhapItem
    {
        [Inject]
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
                ...
                if(nVLDonDatHangShowcrr!=null)
                    PhanQuyenCheck =await phanQuyenAccess.CreateDonDatHang(ModelAdmin.users);
                ...
                if(donHangTotalShow!=null)
                {
                    if (donHangTotalShow.lstitem == null)
                    {
                        _ = searchMaHangAsync();
                    }
                    else
					  ...
					  where [Index]=1
						...
						SELECT dhitem.[Serial], dhitem.[SerialMaDH], dhitem.[MaHang], dhitem.[SLDatHang], dhitem.SignInt, dhitem.NoiDung,
                                   ...
                                   dhitem.[Group], dhitem.GroupIndex,dhitem.GiaTri,hh.TenHang, NvlNhaCungCap.TenNCC, ISNULL(dhitem.DonGia,0) * IsNull(dhitem.SLDatHang,0) as ThanhTien 
                                    ,isnull(qrytk.SLTon,0) as SLTon,hh.MinTK,hh.MaxTK,dhitem.DuyetItemMsg,nh.PhanLoai,STT
                                   ...
                                   N'NvlDonDatHangItem' as [Group], 0 as [GroupIndex],GiaTri,DuyetItemMsg,isnull(STT,0) as STT
        ...
        private async Task searchMaHangAsync()
        {
            PanelVisible = true;
            ...
            try
            {
                ...
                sql = string.Format(@"Use NVLDB
						...
						SELECT dhitem.[Serial], dhitem.[SerialMaDH], dhitem.[MaHang], dhitem.[SLDatHang], dhitem.SignInt, dhitem.NoiDung,
                            ...
                            FROM [NvlDonDatHangItem]
                            Where MaNCC = @MaNCC and MaHang=@MaHang) dhitem 
							left join (select MaHang,sum(SLNhap-SLXuat) as SLTon from NvlNhapXuatItem group by MaHang) as qrytk
							on dhitem.MaHang=qrytk.MaHang
							INNER JOIN NvlHangHoa hh ON hh.MaHang = dhitem.MaHang 
							inner join dbo.NvlNhomHang nh on hh.MaNhom=nh.MaNhom
                ...
                CallAPI callAPI = new CallAPI();
                List<ParameterDefine> lstpara = new List<ParameterDefine>();
                lstpara.Add(new ParameterDefine("@MaHang", donHangTotalShow.MaHang));
                lstpara.Add(new ParameterDefine("@MaNCC", donHangTotalShow.MaNCC));
                string json = await callAPI.ExcuteQueryDatasetEncrypt(sql, lstpara);
                if (json != "")
                {
                    var query = JsonConvert.DeserializeObject<CustomRootItem>(json);
                    if (query != null)
                    {
                        ...
                        {
                            lstdata = new System.Collections.ObjectModel.ObservableCollection<NVLDonDatHangItemShow>(query.lstmuahangitem);
                            donHangTotalShow.lstitem = lstdata;
                        }
                        query.lstmuahangitem.Clear();
                        //query.lstkyduyet.Clear();
                    }
