...
{
    public partial class Urc_KeHoachChuaDatHangDetail
    {
        [Inject]
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
               ...
               // Console.WriteLine(this.GetType().Name);
                ...
                VisibleKeHoachMuaHang = (keHoachMuaHangcrr.LoaiKeHoach.Contains("MuaHang"));
                ...
                if (keHoachMuaHangcrr.LoaiKeHoach.ToLower().Contains("muahang"))
                {
                    ...
                    sql = string.Format(@"Use NVLDB
									...
									declare @tblkh table(ID nvarchar(100),MaSP nvarchar(100),TenSP nvarchar(200),KhuVucKH nvarchar(100),MaMauKH nvarchar(100),SoLuongSP float,TenMau nvarchar(100),Color nvarchar(10))
                                        ...
                                        select qry.*,hh.TenHang,nh.PhanLoai,hh.DVT,isnull(qrytk.SLTon,0) as SLTon,hh.MinTK,hh.MaxTK,tblkh.TenSP,'' TenDinhMuc,'' as CongDoan,'' as IDKeHoach,qry.KeyGroup,cast(tblkh.SoLuongSP as int) as SoLuongSP,tblkh.TenMau,tblkh.Color from 
                                                (SELECT  [Serial],[SerialDN],[MaHang],MaSP,isnull(STT,0) as STT
                ...
                if (keHoachMuaHangcrr.LoaiKeHoach == "DeNghiXuatHangTheoKHSX" || keHoachMuaHangcrr.LoaiKeHoach == "DeNghiXuatHang")
                {
                    sql = string.Format(@"Use NVLDB
                                    ...
                                    GROUP BY SerialLink, TableName, KeyGroup, TenDinhMuc, CongDoan
                                        ...
                                        select qry.*,hh.TenHang,nh.PhanLoai,hh.DVT,isnull(qrytk.SLTon,0) as SLTon,hh.MinTK,hh.MaxTK,tblkh.TenSP,khdm.TenDinhMuc,khdm.CongDoan,khdm.IDKeHoach,qry.KeyGroup,cast(khdm.SoLuong as int) as SoLuongSP,tblkh.TenMau,tblkh.Color from 
                                                (SELECT  [Serial],[SerialDN],[MaHang],MaSP,isnull(STT,0) as STT
                                                          ,[SoLuong],SLTheoDoi,[DonGia],SLHuy,[VAT],[GhiChu],NgayEdit,[NgayInsert],[UserInsert] ,[SerialLink],[TableName],DuyetItemMsg,HuyDatHang,isnull(TenLienKet,'') as TenLienKet,KeyGroup
                        ...
                        if (query.lstmuahangitem != null)
                        {
                            ...
                            if (keHoachMuaHangcrr.isChanged)
                            {
                                foreach (var it in query.lstmuahangitem)
                                {
                                    ...
                                    it.ThanhTien = it.SLDatHang * it.DonGia;
                                }
