...
{
    public partial class Page_KeHoachChuaDatHang_Group
    {
        [Inject]
        ...
        private async Task loaddatadropdownAsync()
        {
            try
            {
                ...
                dieuKienTimKiem.NguoiDN = ModelAdmin.users.TenUser;
        ...
        public async void searchAsync()
        {
            ...
            sql = string.Format(@"
                    ...
                    declare @dtloinhan table(SerialLink int Primary Key)
					insert into @dtloinhan(SerialLink)
					select SerialLink from NvlMsg where SerialLink in (select SerialDN from @tbltheodoi) and TableName='NvlKeHoachMuaHang' group by SerialLink
                    ...
                    group by SerialDN
                        ...
                        ,case when ln.SerialLink is null then '' else N'Có tin nhắn'  end as Msg
                         from 
                        ...
                        left join @dtloinhan ln on ddh.Serial=ln.SerialLink
        ...
        private void ShowMsgChat()
        {

            renderFragment = builder =>
            {
                builder.OpenComponent<MsgChat>(0);
                //builder.AddAttribute(1, "nVLDonDatHangcrr", nVLDonDatHangShow);
                //builder.AddAttribute(2, "CallBackAfterSave", EventCallback.Factory.Create<int>(this, SearchDeNghi));
                builder.CloseComponent();
            };
